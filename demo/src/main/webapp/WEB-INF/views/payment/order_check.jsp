						...
						<table summary="주문 제품 확인 게시판으로 상품명, 가격, 수량, 합계순으로 조회 하실수 있습니다." class="orderTable" border="1" cellspacing="0">
							...
							<tbody>
								...
								<tr>
									...
									<td>${p_count} 개</td>
									<td>${medi.price*p_count}원</td> <!-- 가격 총 합계 -->  
								</tr>
										</c:if>
								...
								<tr>
									...
									<td>${p_count}개</td>
									<td>${daily.price*p_count}원</td> <!-- 가격 총 합계 -->  
								</tr>
							</tbody>
						</table>
					</div>
					...
					<div class="amount">
						...
						<ul class="info">
							<li>
								...
								<!-- medi 에 값이  없을때 -->
								<c:if test="${medi != null }">
								<span class="won"><strong>${medi.price*p_count}</strong> 원</span>
								</c:if>
								<!-- household 에 값이 없을때 -->
						        <c:if test="${medi == null }">
								<span class="won"><strong>${daily.price*p_count}</strong> 원</span>
								</c:if>
							</li>
							<li>
						...
						<ul class="total">
							...
							<li class="txt"><strong>결제 예정 금액</strong></li>
								<!-- medi 에 값이  없을때 -->
								<c:if test="${medi != null }">
								<li class="money"><span>${medi.price*p_count +2500}</span> 원</li>
								</c:if>
								<!-- household 에 값이 없을때 -->
						        <c:if test="${medi == null }">
								<li class="money"><span>${daily.price*p_count +2500}</span> 원</li>
								</c:if>
						</ul>
					</div>
			<!-- //총 주문금액 -->
						...
						<table summary="결제되는 금액를 총 주문금액, 쿠폰할인, 배송비, 포인트사용, 총 결제금액 순으로 확인 하실수 있습니다." class="checkTableM" border="1" cellspacing="0">
							...
							<tbody>
								<tr>
									...
									<c:if test="${medi != null }">
									<td>${medi.price*p_count}원</td>
									</c:if>
									
									<c:if test="${medi == null }">
									<td>${daily.price*p_count}원</td>
									</c:if>
								...
								<tr>
									<th scope="row"><span>총 결제<u>금액</u></span></th>
										<c:if test="${medi != null }">
									<td colspan="3"><strong>${medi.price*p_count +2500} 원</strong></td>
									</c:if>
									<c:if test="${medi == null }">
									<td colspan="3"><strong>${daily.price*p_count +2500} 원</strong></td>
									</c:if>
								</tr>
							</tbody>
						</table>
