			    ...
			    (response) => {  // PC 환경에서 결제 프로세스 완료 후 실행 될 로직
			    	console.log(response);
			         if (response.error_code == "F400") { 
			              ...
			              // URL 생성
			              var url = "/payment/order_check2?p_num=" + p_num + "&p_count=" + count + "&name=" + encodeURIComponent(name) + "&zip=" + zip + "&zip1=" + zip1 + "&zip2=" + zip2 + "&phone="+phone +"&email="+email+"&order_no="+order_no;
			              // 링크 리다이렉트
			              location.href = url;
			      } 
